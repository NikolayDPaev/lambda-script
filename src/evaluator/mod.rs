mod operations;
#[cfg(test)]
mod tests;
use by_address::ByAddress;
use rpds::HashTrieMap;
use std::borrow::BorrowMut;
use std::cell::RefCell;
use std::{collections::HashMap, rc::Rc};

use crate::evaluator::operations::*;
use crate::parser::enums::*;

#[derive(Debug)]
pub enum EvaluatorError {
    UnknownName(Rc<RefCell<Expression>>),
    FunctionExpected(Rc<RefCell<Expression>>),
    ArgsAndParamsLensMismatch(Rc<RefCell<Expression>>),
    InvalidOperation {
        msg: String,
        expr: Rc<RefCell<Expression>>,
    },
    SideEffectInPureScope(Rc<RefCell<Expression>>),
    ConditionShouldEvaluateToBoolean(Rc<RefCell<Expression>>),
    ComparisonError {
        op: CmpBinOp,
        value_1: Value,
        value_2: Value,
    },
    ArithmeticError {
        op: ArithBinOp,
        value_1: Value,
        value_2: Value,
    },
}

pub struct Evaluator {
    memoization_map: HashMap<ByAddress<Rc<RefCell<Expression>>>, Rc<RefCell<Expression>>>,
}

macro_rules! add_outside_assignments {
    ($outside_assignments: expr, $assignments: expr) => {
        $assignments
            .iter()
            .fold($outside_assignments.clone(), |acc, (key, value)| {
                acc.insert(key.clone(), value.clone())
            })
    };
}

macro_rules! make_thunk {
    ($expr:expr, $assignments: expr, $memoize: expr) => {
        Rc::new(RefCell::new(Expression::Thunk(
            $expr.clone(),
            $assignments.clone(),
            $memoize,
        )))
    };
}

impl Evaluator {
    pub fn new() -> Evaluator {
        return Evaluator {
            memoization_map: HashMap::new(),
        };
    }

    pub fn eval_outside_scope(&mut self, scope: &Scope) -> Result<Value, EvaluatorError> {
        let mut expression = self.eval_scope(scope, HashTrieMap::new())?;
        loop {
            if let Expression::Value(value) = &*expression.borrow() {
                return Ok(value.clone());
            }

            expression = self.eval_expression(expression, HashTrieMap::new(), false)?;
        }
    }

    fn force_eval(
        &mut self,
        expr: Rc<RefCell<Expression>>,
        assignments: HashTrieMap<String, Rc<RefCell<Expression>>>,
        memoize: bool,
    ) -> Result<Value, EvaluatorError> {
        let mut expression = expr.clone();
        loop {
            if let Expression::Value(value) = &*expression.borrow() {
                // if memoize && matches!(expr.as_ref(), Expression::FunctionCall{ .. }) {
                //     self.memoization_map.insert(ByAddress(expr), Rc::new(Expression::Value(value.clone())));
                // }
                return Ok(value.clone());
            }
            //println!("Evaluating expression: {:?}", expression);
            expression = self.eval_expression(expression, assignments.clone(), memoize)?
        }
    }

    fn eval_scope(
        &mut self,
        scope: &Scope,
        outside_assignments: HashTrieMap<String, Rc<RefCell<Expression>>>,
    ) -> Result<Rc<RefCell<Expression>>, EvaluatorError> {
        match scope {
            Scope::Pure {
                assignments,
                expression,
            } => {
                let assignments_map = add_outside_assignments!(outside_assignments, assignments);

                Ok(make_thunk!(expression.clone(), assignments_map, true))
            }
            Scope::NonPure {
                assignments,
                statements,
            } => {
                let assignments_map = add_outside_assignments!(outside_assignments, assignments);
                let mut last_expr = Rc::new(RefCell::new(Expression::Value(Value::Nil)));
                for expr in statements {
                    last_expr = Rc::new(RefCell::new(Expression::Value(self.force_eval(
                        expr.clone(),
                        assignments_map.clone(),
                        false,
                    )?)));
                }
                Ok(last_expr)
            }
        }
    }

    fn eval_expression(
        &mut self,
        expr: Rc<RefCell<Expression>>,
        assignments: HashTrieMap<String, Rc<RefCell<Expression>>>,
        memoize: bool,
    ) -> Result<Rc<RefCell<Expression>>, EvaluatorError> {
        let temp = Expression::Value(Value::Nil);
        let inside_rc = expr.replace(temp);

        let result = match &inside_rc {
            Expression::Value(_) => expr.clone(),
            Expression::Thunk(inside_expr, env, memoize) => {
                let result = self.eval_expression(inside_expr.clone(), env.clone(), *memoize)?;
                // if matches!(*expr.borrow(), Expression::ReadCall) {
                //     self.memoization_map
                //         .insert(ByAddress(expr.clone()), result.clone());
                // }
                result
            }
            Expression::Name(string) => {
                if let Some(expr) = assignments.get(string) {
                    //self.eval_expression(expr.clone(), assignments, memoize)?
                    self.eval_expression(expr.clone(), assignments, memoize)?
                } else {
                    return Err(EvaluatorError::UnknownName(expr.clone()));
                }
            }
            Expression::FunctionCall { name, args } => {
                match self.force_eval(name.clone(), assignments.clone(), memoize)? {
                    Value::Function { params, scope } => {
                        if memoize && matches!(*scope.borrow(), Scope::NonPure { .. }) {
                            return Err(EvaluatorError::SideEffectInPureScope(name.clone()));
                        }
                        if args.len() != params.len() {
                            return Err(EvaluatorError::ArgsAndParamsLensMismatch(name.clone()));
                        }
                        let assignments = params.into_iter().zip(args.into_iter()).fold(
                            assignments.clone(),
                            |acc, (string, expr)| {
                                acc.insert(
                                    string.clone(),
                                    make_thunk!(
                                        expr,
                                        assignments,
                                        matches!(*scope.borrow(), Scope::NonPure { .. })
                                    ),
                                )
                            },
                        );
                        self.eval_scope(&*scope.borrow(), assignments)?
                    }
                    _ => {
                        return Err(EvaluatorError::FunctionExpected(name.clone()));
                    }
                }
            }
            Expression::ReadCall => {
                if memoize {
                    return Err(EvaluatorError::SideEffectInPureScope(expr.clone()));
                }
                if let Some(expression) = self.memoization_map.get(&ByAddress(expr.clone())) {
                    //println!("using memoization for: {:?}", expression);
                    return Ok(expression.clone());
                }
                let mut string = String::new();
                std::io::stdin().read_line(&mut string).unwrap();

                if string.ends_with('\n') {
                    string.pop();
                    if string.ends_with('\r') {
                        string.pop();
                    }
                }
                Rc::new(RefCell::new(Expression::Value(crate::parser::parse_string(&string))))
            }
            Expression::PrintCall(inside_expr) => {
                if memoize {
                    return Err(EvaluatorError::SideEffectInPureScope(expr.clone()));
                }
                let value = self.force_eval(inside_expr.clone(), assignments, memoize)?;
                print(&value);
                println!();
                Rc::new(RefCell::new(Expression::Value(Value::Nil)))
            }
            Expression::Cons(left, right) => match (&*left.borrow(), &*right.borrow()) {
                (Expression::Value(left), Expression::Value(right)) => Rc::new(RefCell::new(Expression::Value(
                    Value::Tuple(Box::new(left.clone()), Box::new(right.clone())),
                ))),
                (Expression::Value(_), Expression::Thunk(..)) => Rc::new(RefCell::new(Expression::Cons(
                    left.clone(),
                    self.eval_expression(right.clone(), assignments, memoize)?,
                ))),
                (Expression::Thunk(..), Expression::Value(_)) => Rc::new(RefCell::new(Expression::Cons(
                    self.eval_expression(left.clone(), assignments, memoize)?,
                    right.clone(),
                ))),
                (Expression::Thunk(..), Expression::Thunk(..)) => Rc::new(RefCell::new(Expression::Cons(
                    self.eval_expression(left.clone(), assignments.clone(), memoize)?,
                    self.eval_expression(right.clone(), assignments, memoize)?,
                ))),
                (Expression::Value(_), _) => Rc::new(RefCell::new(Expression::Cons(
                    left.clone(),
                    make_thunk!(right.clone(), assignments, memoize),
                ))),
                (_, Expression::Value(_)) => Rc::new(RefCell::new(Expression::Cons(
                    make_thunk!(left.clone(), assignments, memoize),
                    right.clone(),
                ))),
                (_, _) => Rc::new(RefCell::new(Expression::Cons(
                    make_thunk!(left.clone(), assignments.clone(), memoize),
                    make_thunk!(right.clone(), assignments, memoize),
                ))),
            },
            Expression::Left(inside_expr) => match &*inside_expr.borrow() {
                Expression::Cons(left, _) => left.clone(),
                Expression::Value(Value::Tuple(left, _)) => {
                    Rc::new(RefCell::new(Expression::Value(*left.clone())))
                }
                Expression::Value(_) => {
                    return Err(EvaluatorError::InvalidOperation {
                        msg: String::from("Left is defined only for cons and tuple"),
                        expr: expr.clone(),
                    })
                }
                _ => Rc::new(RefCell::new(Expression::Left(self.eval_expression(
                    inside_expr.clone(),
                    assignments,
                    memoize,
                )?))),
            },
            Expression::Right(inside_expr) => match &*inside_expr.borrow() {
                Expression::Cons(_, right) => right.clone(),
                Expression::Value(Value::Tuple(_, right)) => {
                    Rc::new(RefCell::new(Expression::Value(*right.clone())))
                }
                Expression::Value(_) => {
                    return Err(EvaluatorError::InvalidOperation {
                        msg: String::from("Right is defined only for cons and tuple"),
                        expr: expr.clone(),
                    })
                }
                _ => Rc::new(RefCell::new(Expression::Right(self.eval_expression(
                    inside_expr.clone(),
                    assignments,
                    memoize,
                )?))),
            },
            Expression::Empty(inside_expr) => match *inside_expr.borrow() {
                Expression::Value(Value::Nil) => Rc::new(RefCell::new(Expression::Value(Value::Boolean(true)))),
                Expression::Cons(_, _) => Rc::new(RefCell::new(Expression::Value(Value::Boolean(false)))),
                Expression::Value(Value::Tuple(_, _)) => {
                    Rc::new(RefCell::new(Expression::Value(Value::Boolean(false))))
                }
                Expression::Value(_) => {
                    return Err(EvaluatorError::InvalidOperation {
                        msg: String::from("Empty is defined only for cons and tuple"),
                        expr: expr.clone(),
                    })
                }
                _ => Rc::new(RefCell::new(Expression::Empty(self.eval_expression(
                    inside_expr.clone(),
                    assignments,
                    memoize,
                )?))),
            },
            Expression::UnaryOperation(op, inside_expr) => {
                Rc::new(RefCell::new(Expression::Value(eval_unary_op(
                    *op,
                    &self.force_eval(inside_expr.clone(), assignments.clone(), memoize)?,
                )?)))
            }
            Expression::BinaryOperation(op, left, right) => {
                Rc::new(RefCell::new(Expression::Value(eval_bin_op(
                    *op,
                    &self.force_eval(left.clone(), assignments.clone(), memoize)?,
                    &self.force_eval(right.clone(), assignments.clone(), memoize)?,
                )?)))
            }
            Expression::If {
                condition,
                then_scope,
                else_scope,
            } => match self.force_eval(condition.clone(), assignments.clone(), memoize)? {
                Value::Boolean(true) => self.eval_scope(&*then_scope.borrow(), assignments)?,
                Value::Boolean(false) => self.eval_scope(&*else_scope.borrow(), assignments)?,
                _ => {
                    return Err(EvaluatorError::ConditionShouldEvaluateToBoolean(
                        expr.clone(),
                    ))
                }
            },
        };

        if memoize {
            let result_expr = result.replace(Expression::Value(Value::Nil));
            expr.replace(result_expr);
            return Ok(expr);
        }
        Ok(result)
    }
}
//}
